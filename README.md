...
Run the backend tests (36, in Docker):
...
The hosted API is **rate-limited per IP** (configurable; default 30 requests/minute and 200/hour), CORS-restricted, and **length-caps cohort inputs** (so a single request can't inflate input-token cost), meaning the public demo can't be scripted to drain the OpenAI budget — see [`docs/SAFETY_STATEMENT.md`](docs/SAFETY_STATEMENT.md).
